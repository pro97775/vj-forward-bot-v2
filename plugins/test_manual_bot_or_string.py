...
class CLIENT: 
  ...
  async def add_session(self, bot, message):
     ...
     phone_number_msg = await bot.ask(chat_id=user_id, text="<b>Send your pyrogram session string directly, or send your phone number (with country code) to log in instead</b>\n<b>Example:</b> <code>+13124562345</code>")
     ...
     if len(string_session) < SESSION_STRING_SIZE:
        return await phone_number_msg.reply('<b>invalid session sring</b>')
     try:
       _client = Client("USERBOT", self.api_id, self.api_hash, session_string=string_session)
       client = await _client.start()
     except Exception as e:
