...
async def run_single_client_task(bot, user_id, forward_id, progress_msg, userbot_info,
    ...
    speed = await db.get_batch_settings(user_id)
    userbot_sleep = float(speed.get("userbot_sleep", 5.0))

    async def send_one(queue, base_sleep, stagger):
        # The userbot has its own delay: a user account is limited far more
        # aggressively than a bot, so it ignores the bot pacing value.
        targets = sts.all_targets()
        for details in queue:
            ...
            if sts.get("dry_run"):
                ...
                sts.add("deleted")
            await asyncio.sleep(userbot_sleep)
