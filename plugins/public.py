...
async def run(bot, message):
    ...
    bot_sleep = float(bs.get("base_sleep", 3.0))
    ub_sleep = float(bs.get("userbot_sleep", 5.0))
    bot_rate = round(60 / bot_sleep) if bot_sleep > 0 else 0
    ub_rate = round(60 / ub_sleep) if ub_sleep > 0 else 0
    speed_text = (
        f"\n<b>⚡ Estimated Speed:</b> <code>~{len(bots) * bot_rate + (ub_rate if userbot else 0)} msgs/min</code>"
        if active_count > 1
        else ""
    )
    ...
    if bots:
        engine_buttons.append([
            InlineKeyboardButton(
                f"🤖 Bots ({len(bots)}) — ~{len(bots) * bot_rate}/min",
