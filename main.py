...
async def set_commands(bot):
    """Publish the command menu so Telegram shows it in the "/" autocomplete.

    Two scopes are used so the owner-only commands are not advertised to
    everyone: the public list goes to all private chats, and the owner gets the
    public list plus the maintenance commands in their own chat.
    """
    ...
    public = [
        BotCommand("start", "Start the bot / show the help panel"),
        BotCommand("forward", "Start a new forwarding task"),
        BotCommand("settings", "Bots, targets, filters, caption and speed"),
        BotCommand("watch", "Continuously copy new posts from a channel"),
        BotCommand("watches", "List and stop active watches"),
        BotCommand("unequify", "Delete duplicate files from a chat"),
        BotCommand("tasks", "Show your recent task history"),
        BotCommand("stop", "Cancel your running task"),
        BotCommand("reset", "Reset your settings to the defaults"),
    ]
    owner = public + [
        BotCommand("memory", "Owner: memory and duplicate-store report"),
        BotCommand("sweep", "Owner: drop stale in-memory task states"),
        BotCommand("broadcast", "Owner: reply to a message to broadcast it"),
        BotCommand("resetall", "Owner: clear every saved MongoDB URI"),
        BotCommand("restart", "Owner: restart the process"),
    ]

    try:
        await bot.set_bot_commands(public, scope=BotCommandScopeAllPrivateChats())
        if Config.BOT_OWNER:
            await bot.set_bot_commands(
                owner, scope=BotCommandScopeChat(chat_id=Config.BOT_OWNER)
            )
        logger.info("Registered %s public and %s owner commands", len(public), len(owner))
    except Exception as exc:
        # A failure here must never stop the bot from serving.
